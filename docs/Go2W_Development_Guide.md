...
对于“前进固定距离”或“转动固定角度”的导航 primitive，不建议仅按
`速度 × 时间` 推算位移。更稳定的通用做法是：

1. 从 `SportModeState` 保存动作开始时的 position、yaw、velocity 和 yaw rate。
2. 根据相对距离或角度生成目标位姿。
3. 以固定频率计算 position/yaw error，并用 P/PD controller 生成 `Move(vx, 0, vyaw)`。
4. 误差进入容差范围后发送零速度并调用 `StopMove()`。
5. odometry 超时、控制超时或异常时立即走本地停止路径。

只有 P、D 项的实现严格来说是 PD，不应因为类名写成 PID 就假定存在积分项。控制增益、
速度上限、容差和频率必须针对具体地面、载荷及固件低速标定；不要直接复制其他项目的参数。
首次标定应降低速度上限，并保留遥控器急停。
