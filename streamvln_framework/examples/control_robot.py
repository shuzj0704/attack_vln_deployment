#!/usr/bin/env python3
"""Send one StreamVLN primitive to the Robot example service."""
...
from streamvln_framework.protocol import VALID_ACTIONS
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        response = requests.post(
            f"{server_url}/action",
            json={"action": args.action, "request_id": str(uuid.uuid4())},
            timeout=args.timeout,
        )
        response.raise_for_status()
        payload = response.json()
    except (requests.RequestException, ValueError) as exc:
        print(f"StreamVLN Robot action failed: {exc}", file=sys.stderr)
        return 1
    if (
        not isinstance(payload, dict)
        or payload.get("action") != args.action
